...
const Layout: React.FC<LayoutProps> = ({ 
          ...
          <div className="mb-6 flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            <div className="flex items-center gap-4">
              ...
              <button 
                ...
                className={`p-2 rounded-lg text-gray-500 hover:bg-gray-100 transition-all ${currentView === 'Calendar' ? 'text-indigo-600 bg-indigo-50' : ''}`}
                title="Calendar"
              >
              ...
              <button 
                ...
                className={`p-2 rounded-lg text-gray-500 hover:bg-gray-100 transition-all ${currentView === 'Reports' ? 'text-indigo-600 bg-indigo-50' : ''}`}
                title="Reports"
              >
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="bg-indigo-700 text-white shadow-lg sticky top-0 z-50 print:hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center space-x-2 cursor-pointer" onClick={() => onViewChange('Calendar')}>
            <i className="fa-solid fa-calendar-days text-2xl"></i>
            <h1 className="text-xl font-bold tracking-tight">Glenn Anthony</h1>
          </div>
          
          <div className="flex bg-indigo-800 p-1 rounded-xl border border-indigo-600/30">
            <button 
              onClick={() => onViewChange('Calendar')}
              className={`px-4 py-2 flex items-center gap-2 rounded-lg transition-all text-xs font-bold ${currentView === 'Calendar' ? 'bg-white text-indigo-700 shadow-sm' : 'text-indigo-100 hover:bg-indigo-600'}`}
            >
            ...
            <button 
              onClick={() => onViewChange('Reports')}
              className={`px-4 py-2 flex items-center gap-2 rounded-lg transition-all text-xs font-bold ${currentView === 'Reports' ? 'bg-white text-indigo-700 shadow-sm' : 'text-indigo-100 hover:bg-indigo-600'}`}
            >
              <i className="fa-solid fa-file-invoice"></i>
              <span>Reports</span>
            </button>
            <button 
              onClick={() => onViewChange('Settings')}
              className={`px-4 py-2 flex items-center gap-2 rounded-lg transition-all text-xs font-bold ${currentView === 'Settings' ? 'bg-white text-indigo-700 shadow-sm' : 'text-indigo-100 hover:bg-indigo-600'}`}
            >
              <i className="fa-solid fa-gear"></i>
              <span>Settings</span>
            </button>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 pl-4 border-l border-indigo-600/50">
              ...
              <button 
                onClick={onLogout}
                className="w-9 h-9 flex items-center justify-center rounded-xl bg-indigo-800 text-indigo-100 hover:bg-red-500 hover:text-white transition-all shadow-sm"
                title="Logout"
              >
                <i className="fa-solid fa-right-from-bracket"></i>
              </button>
